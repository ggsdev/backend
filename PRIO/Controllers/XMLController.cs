...
namespace PRIO.Controllers
{
    ...
    public class XMLController : ControllerBase
    {
        ...
        public async Task<ActionResult> XMLTeste([FromForm] List<IFormFile> files, [FromServices] DataContext context)
        {
                ...
                var validInput = new List<string>()
            {
                ...
                if (fileType == "039")
                {
                    ...
                    if (dadosBasicosElement is null)
                        return BadRequest(new ErrorResponseDTO
                        {
                            Message = "Dados basicos XML element can't be null, XML bad formated"
                        });
                ...
                if (fileType == "001")
                {
                    ...
                    for (int k = 0; k < dadosBasicosElements.Count(); ++k)
                    {
                        ...
                        var configuracaoCv = Functions.DeserializeXml<CONFIGURACAO_CV_001>(configuracaoCvElement);

                        var elementoPrimarioElement = dadosBasicosElement.Element("LISTA_ELEMENTO_PRIMARIO")?.Element("ELEMENTO_PRIMARIO");
                        var elementoPrimario = Functions.DeserializeXml<ELEMENTO_PRIMARIO_001>(elementoPrimarioElement);

                        var instrumentoPressaoElement = dadosBasicosElement.Element("LISTA_INSTRUMENTO_PRESSAO")?.Element("INSTRUMENTO_PRESSAO");
                        var instrumentoPressao = Functions.DeserializeXml<INSTRUMENTO_PRESSAO_001>(instrumentoPressaoElement);
                        ...
                        var instrumentoTemperatura = Functions.DeserializeXml<INSTRUMENTO_TEMPERATURA_001>(instrumentoTemperaturaElement);

                        var producaoElement = dadosBasicosElement.Element("LISTA_PRODUCAO")?.Element("PRODUCAO");
                        var producao = Functions.DeserializeXml<PRODUCAO_001>(producaoElement);
                ...
                if (fileType == "002")
                {
                    ...
                    for (int k = 0; k < dadosBasicosElements.Count(); ++k)
                    {
                        ...
                        var configuracaoCvElement = dadosBasicosElement?.Element("LISTA_CONFIGURACAO_CV")?.Element("CONFIGURACAO_CV");
                        var configuracaoCv = Functions.DeserializeXml<CONFIGURACAO_CV_002>(configuracaoCvElement);

                        var elementoPrimarioElement = dadosBasicosElement?.Element("LISTA_ELEMENTO_PRIMARIO")?.Element("ELEMENTO_PRIMARIO");
                        var elementoPrimario = Functions.DeserializeXml<ELEMENTO_PRIMARIO_002>(elementoPrimarioElement);

                        var instrumentoPressaoElement = dadosBasicosElement?.Element("LISTA_INSTRUMENTO_PRESSAO")?.Element("INSTRUMENTO_PRESSAO");
                        var instrumentoPressao = Functions.DeserializeXml<INSTRUMENTO_PRESSAO_002>(instrumentoPressaoElement);

                        var instrumentoTemperaturaElement = dadosBasicosElement?.Element("LISTA_INSTRUMENTO_TEMPERATURA")?.Element("INSTRUMENTO_TEMPERATURA");
                        var instrumentoTemperatura = Functions.DeserializeXml<INSTRUMENTO_TEMPERATURA_002>(instrumentoTemperaturaElement);

                        var producaoElement = dadosBasicosElement?.Element("LISTA_PRODUCAO")?.Element("PRODUCAO");
                        var producao = Functions.DeserializeXml<PRODUCAO_002>(producaoElement);
                ...
                if (fileType == "003")
                {
                    ...
                    for (int k = 0; k < dadosBasicosElements.Count(); ++k)
                    {
                        INST_DIFEREN_PRESSAO_ALTA_003? instDiferenPressaoAlta = null;
                        INST_DIFEREN_PRESSAO_MEDIA_003? instDiferenPressaoMedia = null;
                        INST_DIFEREN_PRESSAO_BAIXA_003? instDiferenPressaoBaixa = null;
                        INST_DIFEREN_PRESSAO_PRINCIPAL_003? instDiferenPressaoPrincipal = null;

                        var dadosBasicosElement = dadosBasicosElements.ElementAt(k);
                        var dadosBasicos = Functions.DeserializeXml<DADOS_BASICOS_003>(dadosBasicosElement);

                        var configuracaoCvElement = dadosBasicosElement?.Element("LISTA_CONFIGURACAO_CV")?.Element("CONFIGURACAO_CV");
                        var configuracaoCv = Functions.DeserializeXml<CONFIGURACAO_CV_003>(configuracaoCvElement);

                        var elementoPrimarioElement = dadosBasicosElement?.Element("LISTA_ELEMENTO_PRIMARIO")?.Element("ELEMENTO_PRIMARIO");
                        var elementoPrimario = Functions.DeserializeXml<ELEMENTO_PRIMARIO_003>(elementoPrimarioElement);

                        var instrumentoPressaoElement = dadosBasicosElement?.Element("LISTA_INSTRUMENTO_PRESSAO")?.Element("INSTRUMENTO_PRESSAO");
                        var instrumentoPressao = Functions.DeserializeXml<INSTRUMENTO_PRESSAO_003>(instrumentoPressaoElement);

                        var instrumentoTemperaturaElement = dadosBasicosElement?.Element("LISTA_INSTRUMENTO_TEMPERATURA")?.Element("INSTRUMENTO_TEMPERATURA");
                        var instrumentoTemperatura = Functions.DeserializeXml<INSTRUMENTO_TEMPERATURA_003>(instrumentoTemperaturaElement);

                        var placaOrificioElement = dadosBasicosElement?.Element("LISTA_PLACA_ORIFICIO")?.Element("PLACA_ORIFICIO");
                        var placaOrificio = Functions.DeserializeXml<PLACA_ORIFICIO_003>(placaOrificioElement);
                        ...
                        if (instDiferenPressaoAltaElement is not null)
                            instDiferenPressaoAlta = Functions.DeserializeXml<INST_DIFEREN_PRESSAO_ALTA_003>(instDiferenPressaoAltaElement);
                        ...
                        if (instDiferenPressaoMediaElement is not null)
                            instDiferenPressaoMedia = Functions.DeserializeXml<INST_DIFEREN_PRESSAO_MEDIA_003>(instDiferenPressaoMediaElement);
                        ...
                        if (instDiferenPressaoBaixaElement is not null)
                            instDiferenPressaoBaixa = Functions.DeserializeXml<INST_DIFEREN_PRESSAO_BAIXA_003>(instDiferenPressaoBaixaElement);
                        ...
                        if (instDiferenPressaoPrincipalElement is not null)
                            instDiferenPressaoPrincipal = Functions.DeserializeXml<INST_DIFEREN_PRESSAO_PRINCIPAL_003>(instDiferenPressaoPrincipalElement);

                        var producaoElement = dadosBasicosElement?.Element("LISTA_PRODUCAO")?.Element("PRODUCAO");
                        var producao = Functions.DeserializeXml<PRODUCAO_003>(producaoElement);
                        #endregion
        ...
        //                IND_HABILITACAO_ALARME_4_003 = instDiferenPressaoPrincipal.IND_HABILITACAO_ALARME_5_003,
        //                MED_PRSO_ADOTADA_FALHA_2_003 = double.TryParse(instDiferenPressaoPrincipal.MED_PRSO_ADOTADA_FALHA_3_003?.Replace(",", "."), NumberStyles.Any, CultureInfo.InvariantCulture, out double MED_PRSO_ADOTADA_FALHA_2_003) ? MED_PRSO_ADOTADA_FALHA_2_003 : null,
        //                DSC_ESTADO_INSNO_CASO_FALHA_2_003 = instDiferenPressaoPrincipal.DSC_ESTADO_INSNO_CASO_FALHA_3_003,
