...
namespace PRIO.src.Modules.FileExport.XML.Infra.Http.Controllers
{
    ...
    public class XMLExportController : ControllerBase
    {
        ...
        [HttpPost("{table}/{id}")]
        public async Task<IActionResult> ExportXML([FromRoute] Guid id, [FromRoute] string table, [FromBody] ExportXMLViewModel body)
        {
            ...
            var xmlDTO = await _XMLService.ExportXML(id, table, body);
            return Ok(xmlDTO);
        }
    }
}
