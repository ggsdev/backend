        ...
        public async Task<WellEventByIdDto> GetClosedEventById(Guid eventId)
        {
            ...
            if (wellEvent.WellLosses is not null)
            {
                foreach (var productionLoss in wellEvent.WellLosses)
                {
                    ...
                    int hours = (int)productionLoss.Downtime;
                    var minutesDecimal = (productionLoss.Downtime - hours) * 60;
                    ...
                    reasonsDetailed.Add(new ReasonDetailedDto
                    {
                        Downtime = formattedTime,
                        EndDate = wellEvent.EndDate is not null ? wellEvent.EndDate.Value.ToString("dd/MM/yyyy : HH/mm") : "",
                        StartDate = wellEvent.StartDate.ToString("dd/MM/yyyy : HH:mm"),
                        ProductionLoss = productionLoss.ProductionLost,
                        //SystemRelated = wellEvent.SystemRelated,
                        TimeOperating = ""
