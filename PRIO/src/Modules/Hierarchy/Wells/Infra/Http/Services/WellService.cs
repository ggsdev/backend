    ...
    public class WellService
    {
        ...
        public async Task<CreateUpdateWellDTO> CreateWell(CreateWellViewModel body, User user)
        {
            ...
            if (well.IsActive)
            {
                var wellEvent = new WellEvent
                {
                    ...
                    IdAutoGenerated = $"{field.Name.Substring(0, 2)}0001",
        ...
        public async Task DeleteWell(Guid id, User user)
        {
            ...
            if (lastEventOfAll is not null && lastEventOfAll.EventStatus.ToUpper() == "A")
            {
                ...
                if (lastEventOfTypeClosing is not null && int.TryParse(lastEventOfTypeClosing.IdAutoGenerated, out lastCode))
                {
                    lastCode++;
                ...
                {
                    Id = Guid.NewGuid(),
                    StartDate = propertiesUpdated.DeletedAt,
                    IdAutoGenerated = $"{well.Field?.Name?.Substring(0, 3)}{codeSequencial}",
                    Well = well,
                    EventStatus = "F",
                    StateANP = "4",
                    StatusANP = "Fechado"
                ...
                lastEventOfAll.Interval = (propertiesUpdated.DeletedAt - lastEventOfAll.StartDate).TotalHours;
                lastEventOfAll.EndDate = propertiesUpdated.DeletedAt;
